...
IMG=soaringls/ubuntu16:191010  #commit by ls:所用镜像

function local_volumes() {
    # Apollo root and bazel cache dirs are required.
    volumes="-v ${AUTOBOT_ROOT_DIR}:/autobot/catkin_ws"  #commit by ls:/autobot/catkin_ws等同于${AUTOBOT_ROOT_DIR},包含内容一致
...
function main(){
    ...
    ${DOCKER_CMD} run -it \
        ...
        $(local_volumes) \
        --net host \
        -w /autobot \  ##commit by ls:进入docker容器后所在默认路径  #setting by self
