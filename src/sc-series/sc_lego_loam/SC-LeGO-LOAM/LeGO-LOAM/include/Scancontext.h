...
using PointT = pcl::PointXYZI; // using xyz only. but a user can exchange the original bin encoding function (i.e., max hegiht) to max intensity (for detail, refer 20 ICRA Intensity Scan Context)
...
public: 
    ...
    // User-side API
    void makeAndSaveScancontextAndKeys( pcl::PointCloud<PointT> & _scan_down );
...
public:
    ...
    const double PC_UNIT_SECTORANGLE = 360.0 / double(PC_NUM_SECTOR);// = 360/60
    ...
    std::vector<Eigen::MatrixXd> polarcontexts_;//<rows*cols-->20*60>
    std::vector<Eigen::MatrixXd> polarcontext_invkeys_;//<rows*1->20*1>
    std::vector<Eigen::MatrixXd> polarcontext_vkeys_;//<1×cols->1*60>
    KeyMat polarcontext_invkeys_mat_;//std::vector<std::vector<float> >, rings-mean's value

    KeyMat polarcontext_invkeys_to_search_;
    std::unique_ptr<InvKeyTree> polarcontext_tree_;//KDTreeVectorOfVectorsAdaptor< KeyMat, float >;
