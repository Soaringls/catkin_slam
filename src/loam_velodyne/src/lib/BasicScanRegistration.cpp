...
/**
 * @scanTime 当前帧点云的起点时间戳
 * @laserCloudScans 如16线雷达  存储每根scanline的点云
 */ 
...
void BasicScanRegistration::updateIMUData(Vector3& acc, IMUState& newState)
{
  if (_imuHistory.size() > 0) {
    ...
    rotateZXY(acc, newState.roll, newState.pitch, newState.yaw);//lidar
    //todo
...
/**
 * @relTime 当前point在帧内的时间戳
 * @brief:  对点云补偿imu的位置漂移，补偿后点云的rotation部分为start时刻的lidar坐标系下所观察到的,即恢复静态or消除imu飘造成的畸变
 */ 
void BasicScanRegistration::projectPointToStartOfSweep(pcl::PointXYZI& point, float relTime)//sweep即:一帧点云
{
...
void BasicScanRegistration::extractFeatures(const uint16_t& beginIdx)
{
  ...
  size_t nScans = _scanIndices.size();//_scanIndices.size = 16  每个元素为每个scanline起始点索引
  for (size_t i = beginIdx; i < nScans; i++) {//遍历每个scanline
    ...
    for (int j = 0; j < _config.nFeatureRegions; j++) {//每个scanline的点划分为6个片段
      ...
      setRegionBuffersFor(sp, ep);//每个scanline的点划分为6个片段，sp和ep为当前片段点的起始索引
      ...
      for (size_t k = regionSize; k > 0 && largestPickedNum < _config.maxCornerLessSharp;) {
        size_t idx = _regionSortIndices[--k];//segment内曲率从大到小遍历
...
//scanline中某segment在整体pointcloud中的起始点索引
void BasicScanRegistration::setRegionBuffersFor(const size_t& startIdx, const size_t& endIdx)
{
  ...
  for (size_t i = startIdx, regionIdx = 0; i <= endIdx; i++, regionIdx++) {
    ...
    //_regionCurvature存储某scanline当前片段中每个点的曲率值
...
void BasicScanRegistration::setScanBuffersFor(const size_t& startIdx, const size_t& endIdx)
{
  ...
  _scanNeighborPicked.assign(scanSize, 0);//当前scanline扫描到的点的个数
