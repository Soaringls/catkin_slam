...
void BasicLaserOdometry::process()
{
   ...
   _frameCount++;
   _transform.pos -= _imuVeloFromStart * _scanPeriod;  //_transform更新 意义？？
   ...
   if (lastCornerCloudSize > 10 && lastSurfaceCloudSize > 100)
   {
      ...
      for (size_t iterCount = 0; iterCount < _maxIterations; iterCount++)
      {
         ...
         for (int i = 0; i < cornerPointsSharpNum; i++)
         {
            ...
            if (iterCount % 5 == 0)
            {
               ...
               if (pointSearchSqDis[0] < 25)
               {
                  ...
                  int closestPointScan = int(_lastCornerCloud->points[closestPointInd].intensity);//最临近点的scanline的id

                  float pointSqDis, minPointSqDis2 = 25;
                  //遍历target-corner中临近点后面的点
               ...
               _pointSearchCornerInd1[i] = closestPointInd;//target-corner index
               _pointSearchCornerInd2[i] = minPointInd2;//cur-corner index
            }
         ...
         //regis2-----遍历当前帧source-surface的每个点
