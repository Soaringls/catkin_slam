/**
 * reference: https://zhuanlan.zhihu.com/p/57351961
 */ 
					...
					pcl::PointCloud<PointType>::Ptr coeff_selected)
{
	...
	//高斯牛顿法, 求解增量方程J* delta_x = -J * f(x)
	//直接解normal equation求步长, QR分解是一种解法
	cv::solve(mat_a_t_a, mat_a_t_b, mat_x, cv::DECOMP_QR); //mat_x 即 delta_x
					 ...
					 )
{
	for (int i = 0; i < corner_feature_points->size(); i++)
	{
		...
		cv::eigen(mat_a1, mat_d1, mat_v1);//特征值mat_d1 1*3   特征向量mat_v1 3*3
		...
		if (mat_d1.at<float>(0, 0) > 3 * mat_d1.at<float>(0, 1))
		{
			...
			//点到直线的距离对source中世界系下点point_sel求导
