...
#include "lib2/cloud_segmentation.h"
#include "lib2/lidar_odometry.h"
#include "lib2/lidar_refination.h"
// #include "lib2/loam_odometry.h"
 ...
 public:
  ...
  void Callback(const sensor_msgs::PointCloud2ConstPtr& input) {
    ...
    publishCloud(&pub_planar_lessflat_, dataset->surfPointsLessFlat,
                 ros::Time(time));
    lidar_odom_.Process();
    // LOG(INFO) << "*********************************LidarOdometr:"
    //           << t_sub.end(true) << " [ms]";
    // if (cnt % 5 == 0) {
    lidar_mapping_.Process();

    // LOG(INFO) << "*********************************LidarRefination:"
    //           << t_sub.end(true) << " [ms]";
    // }

    LOG(INFO) << "final processTotal:" << t_frame.end() << " [ms]";
    // publish odometry
    // auto q_w_curr = lidar_odom_.GetQuaternion();
    // auto t_w_curr = lidar_odom_.GetPos();
    // auto odom_pose = lidar_odom_.GetRefinedPose();
    // static nav_msgs::Path odomPath;
    // publishPath(pub_odom_, odomPath, odom_pose);

    // auto mapq_w_curr = lidar_mapping_.GetQuaternion();
    // auto mapt_w_curr = lidar_mapping_.GetPos();
