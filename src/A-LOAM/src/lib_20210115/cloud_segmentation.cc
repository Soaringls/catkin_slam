...
void CloudSegmentation::divideSurfEdge() {
  ...
  */
  ...
  for (std::size_t i = 0; i < cloud_size - 5; ++i) {
    ...
    cloudCurvature[i] = diff_x * diff_x + diff_y * diff_y + diff_z * diff_z;
    cloudSortInd[i] = i;
  ...
  for (std::size_t i = 0; i < num_scans; ++i) {
    ...
    for (std::size_t j = 0; j < 6; ++j) {
      ...
      for (std::size_t k = ep; k >= sp; k--) {
        int id = cloudSortInd[k];
        if (cloudNeighborPicked[id] == 0 && cloudCurvature[id] > 0.1) {
          largest_picked_num++;
