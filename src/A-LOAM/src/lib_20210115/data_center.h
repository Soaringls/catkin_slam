...
struct ScanSegMsg {
  ...
  ScanSegMsg& operator=(const ScanSegMsg& rhs) {
    ...
    // LOG(INFO) << __func__ << ", this->laserCloud:" <<
    // this->laserCloud->size(); LOG(INFO) << __func__ << ",
    // this->cornerPointsSharp:"
    //           << this->cornerPointsSharp->size();
    // LOG(INFO) << __func__ << ", this->cornerPointsLessSharp:"
    //           << this->cornerPointsLessSharp->size();
    // LOG(INFO) << __func__
    //           << ", this->surfPointsFlat:" << this->surfPointsFlat->size();
    // LOG(INFO) << __func__ << ", this->surfPointsLessFlat:"
    //           << this->surfPointsLessFlat->size();
    // ////////////////
    // LOG(INFO) << __func__ << ", rhs.timestamp:" << rhs.timestamp;
    // LOG(INFO) << __func__ << ", rhs.laserCloud:" << rhs.laserCloud->size();
    // LOG(INFO) << __func__
    //           << ", rhs.cornerPointsSharp:" << rhs.cornerPointsSharp->size();
    // LOG(INFO) << __func__ << ", rhs.cornerPointsLessSharp:"
    //           << rhs.cornerPointsLessSharp->size();
    // LOG(INFO) << __func__
    //           << ", rhs.surfPointsFlat:" << rhs.surfPointsFlat->size();
    // LOG(INFO) << __func__
    //           << ", rhs.surfPointsLessFlat:" <<
    //           rhs.surfPointsLessFlat->size();
    return *this;
  }
};
