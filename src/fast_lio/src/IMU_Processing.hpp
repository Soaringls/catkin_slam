...
class ImuProcess {
 public:
  ...
  V3D cov_bias_gyr;
  V3D cov_bias_acc;
  double first_lidar_time;  // just init: lidar_begin_time of first scan
...
void ImuProcess::IMU_init(
  ...
  if (b_first_frame_ /*default=true*/) {
    Reset();
    N = 1;
  ...
  init_state.grav = S2(-mean_acc / mean_acc.norm() *
                       G_m_s2 /*9.81 Gravaty const in GuangDong/China*/);
...
void ImuProcess::UndistortPcl(
  ...
  imu_poses_.clear();  // vector<Pose6D>
  imu_poses_.push_back(
      set_pose6d(0.0 /*offset_time of Pose6D*/,
                 acc_s_last /*加速度a=R*(linear_acc-ba)- 9.8 即enu系下加速度*/,
                 angvel_last, imu_state.vel, imu_state.pos,
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    acc_avr = acc_avr * G_m_s2 /*9.81*/ / mean_acc.norm();  // - state_inout.ba;
    ...
    in.acc = acc_avr;
    in.gyro = angvel_avr;
    Q.block<3, 3>(0, 0).diagonal() = cov_gyr;  // Q: 12x12
    ...
    angvel_last = angvel_avr - imu_state.bg;  // corrected Wt

    //加速度a=R*(linear_acc-ba)- 9.8 即enu系下加速度
    ...
    imu_poses_.push_back(set_pose6d(offs_t, acc_s_last, angvel_last,
  ...
  for (auto iter = imu_poses_.end() - 1; iter != imu_poses_.begin(); iter--) {
    auto head = iter - 1;
    auto tail = iter;
    ...
    /*condiditon: point is between (head,tail)*/
    ...
    for (; dt > 0; it_pcl--) {
      ...
      M3D R_i(R_imu * Exp(angvel_avr, dt));  // altitude of point "it_pcl"

      V3D P_i(it_pcl->x, it_pcl->y, it_pcl->z);
      V3D T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt -
               imu_state.pos);
      V3D P_compensate =  // offset_R_L_I.conjugate() 等同于求逆
...
void ImuProcess::Process(const MeasureGroup &meas,
  ...
  if (imu_need_init_) {
    ...
    IMU_init(meas, kf_state, init_iter_num /*initial = 1*/);
    ...
    state_ikfom imu_state = kf_state.get_x();
    if (init_iter_num > MAX_INI_COUNT /*20*/) {
