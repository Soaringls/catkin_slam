...
private:
   ...
   */
  std::vector<KeyFrame::Ptr> find_candidates(const std::vector<KeyFrame::Ptr>& keyframes, 
  ...
  /**
   * @brief To validate a loop candidate this function applies a scan matching between keyframes consisting the loop. 
   *        If they are matched well, the loop is added to the pose graph
  ...
  Loop::Ptr matching(const std::vector<KeyFrame::Ptr>& candidate_keyframes, 
    ...
    for(const auto& candidate : candidate_keyframes) {
      ...
      guess(2, 3) = 0.0;
      registration->align(*aligned, guess); //gnss为source_cloud在target中的坐标
      std::cout << "." << std::flush;
      ...
      relative_pose = registration->getFinalTransformation();//t1在t2中的坐标,即source在target中坐标
    }
