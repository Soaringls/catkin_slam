...
public:
  ...
  virtual void onInit() {
    ...
    initialize_params();
    std::string cloud_topic = private_nh.param<std::string>(cloud_topic, "velodyne_points");
    points_sub = nh.subscribe(cloud_topic, 256, &FloorDetectionNodelet::cloud_callback, this);
    floor_pub = nh.advertise<hdl_graph_slam::FloorCoeffs>("/floor_detection/floor_coeffs", 32);
    ROS_INFO_STREAM("FloorDetectionNodelet:points_sub: "<<points_sub);
...
private:
   ...
   * @return
   */
  pcl::PointCloud<PointT>::Ptr plane_clip(const pcl::PointCloud<PointT>::Ptr& src_cloud, 
