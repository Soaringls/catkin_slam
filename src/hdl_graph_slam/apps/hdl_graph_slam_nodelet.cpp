...
private:
  ...
  bool flush_keyframe_queue() {
    ...
    Eigen::Isometry3d odom2map(trans_odom2map.cast<double>()); //trans_odom2map第一次的初始值为单位阵，后面g2o优化
    ...
    for(int i=0; i < std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) {
      ...
      Eigen::Isometry3d odom = odom2map * keyframe->odom;//odom左乘trans_odom2map，变换为优化后estimate的odom
      ...
      Eigen::Isometry3d relative_pose = keyframe->odom.inverse() * prev_keyframe->odom; //tf测量值 t1在t2中的坐标
      //information为6*6关于旋转和平移的协方差矩阵，
      Eigen::MatrixXd information = inf_calclator->calc_information_matrix(keyframe->cloud, 
                                                                           prev_keyframe->cloud, 
                                                                           relative_pose);
      auto edge = graph_slam->add_se3_edge(keyframe->node, prev_keyframe->node, relative_pose, information);
      graph_slam->add_robust_kernel(edge, private_nh.param<std::string>("odometry_edge_robust_kernel", "NONE"), 
  ...
  bool flush_imu_queue() {
    ...
    for(auto& keyframe : keyframes) {
      ...
      keyframe->orientation = Eigen::Quaterniond(quat_base.quaternion.w, quat_base.quaternion.x, 
                                                 quat_base.quaternion.y, quat_base.quaternion.z);
      ...
      if(enable_imu_orientation) {
        ...
        auto edge = graph_slam->add_se3_prior_quat_edge(keyframe->node, 
                                                        *keyframe->orientation, //measurement
                                                        info);
      ...
      if(enable_imu_acceleration) {
        ...
        g2o::OptimizableGraph::Edge* edge = graph_slam->add_se3_prior_vec_edge(keyframe->node, 
                                                                               ...
                                                                               *keyframe->acceleration, //measurement
                                                                               info);
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    ...
    if(anchor_node && private_nh.param<bool>("fix_first_node_adaptive", true)) {//edge-vertices[1] pre  [0]-current
    ...
    trans_odom2map_mutex.lock();
    trans_odom2map = trans.matrix().cast<float>();//odom左乘trans_odom2map，变换为优化后estimate的odom
    trans_odom2map_mutex.unlock();
    ...
    if(odom2map_pub.getNumSubscribers()) {
      geometry_msgs::TransformStamped ts = matrix2transform(keyframe->stamp, trans.matrix().cast<float>(), 
                                                            map_frame_id, odom_frame_id); //map  odom
  ...
  bool save_map_service(hdl_graph_slam::SaveMapRequest& req, hdl_graph_slam::SaveMapResponse& res) {
    ...
    if(zero_utm) {
      ...
      ofs <<std::setiosflags(std::ios::fixed)<<std::setprecision(5)<<zero_utm.get()(0,0)<<"\t"<<zero_utm.get()(1,0)<<"\t"<<zero_utm.get()(2,0)<<std::endl;
    }
