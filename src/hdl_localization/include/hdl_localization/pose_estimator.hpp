...
public:
      ...
      cool_time_duration(cool_time_duration)
  {
    ...
    PoseSystem system;
    ukf.reset(new kkl::alg::UnscentedKalmanFilterX<float, PoseSystem>(system, 
                                                                      16, 6, 7, //state-dim、input-dm、measurement-dim
                                                                      process_noise, 
                                                                      measurement_noise, 
                                                                      mean, cov));//
  }
  ...
  pcl::PointCloud<PointT>::Ptr correct(const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    if(quat().vec().dot(q.vec()) < 0.0f) {//dot 点乘  cross 叉乘
      q.coeffs() *= -1.0f;
    }
