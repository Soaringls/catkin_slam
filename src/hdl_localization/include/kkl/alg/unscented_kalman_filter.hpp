...
public:
   ...
   */
  UnscentedKalmanFilterX(const System& system, 
                         int state_dim, int input_dim, int measurement_dim, //16  6  7
                         const MatrixXt& process_noise, 
                         const MatrixXt& measurement_noise, //测量值p q的协方差阵
                         const VectorXt& mean, const MatrixXt& cov)//p v q ab wb及其协方差阵,其中p q赋予初始值
    : state_dim(state_dim),
    input_dim(input_dim),
    ...
    normal_dist(0.0, 1.0)
  {
    ...
    weights[0] = lambda / (N + lambda);    // 1/17
    for (int i = 1; i < 2 * N + 1; i++) {
      weights[i] = 1 / (2 * (N + lambda)); // 1/34
    }

    // weights for extended state space which includes error variances
    ext_weights[0] = lambda / (N + K + lambda);    // 1/24
    for (int i = 1; i < 2 * (N + K) + 1; i++) {
      ext_weights[i] = 1 / (2 * (N + K + lambda)); // 1/48
    }
  }
  ...
  void predict(const VectorXt& control) {
    ...
    computeSigmaPoints(mean, cov, sigma_points);//提取sigma点,Ref<概率机器人> page-50(3.66)  page-53(2) 
    for (int i = 0; i < S; i++) {//33
      sigma_points.row(i) = system.f(sigma_points.row(i), control);//更新预测每//Ref<概率机器人> page-53(3) 个sigma点(即p v q ab wb)
    }
    ...
    cov_pred.setZero();
    for (int i = 0; i < S; i++) {//33
      mean_pred += weights[i] * sigma_points.row(i);//Ref<概率机器人> page-53(4)
    }
    for (int i = 0; i < S; i++) {//33
      VectorXt diff = sigma_points.row(i).transpose() - mean;
      cov_pred += weights[i] * diff * diff.transpose();//Ref<概率机器人> page-53(5)
    }
    cov_pred += R;
   ...
   */
  void correct(const VectorXt& measurement) {//测量值:位置和姿态
    // create extended state space which includes error variances
    VectorXt ext_mean_pred = VectorXt::Zero(N + K, 1);    //23,1  
    MatrixXt ext_cov_pred = MatrixXt::Zero(N + K, N + K); //23,23 
    ...
    computeSigmaPoints(ext_mean_pred, ext_cov_pred, ext_sigma_points);//Ref<概率机器人> page-53(6)

    //------expected_measurement_mean
    // unscented transform
    expected_measurements.setZero();//47,7
    for (int i = 0; i < ext_sigma_points.rows(); i++) {//47,23
      expected_measurements.row(i) = system.h(ext_sigma_points.row(i).transpose().topLeftCorner(N, 1));//Ref<概率机器人> page-53(7)
      expected_measurements.row(i) += VectorXt(ext_sigma_points.row(i).transpose().bottomRightCorner(K, 1));
    }

    VectorXt expected_measurement_mean = VectorXt::Zero(K);//K=7
    for (int i = 0; i < ext_sigma_points.rows(); i++) {
      expected_measurement_mean += ext_weights[i] * expected_measurements.row(i);//Ref<概率机器人> page-53(8)
    }
    ...
    for (int i = 0; i < ext_sigma_points.rows(); i++) {
      ...
      expected_measurement_cov += ext_weights[i] * diff * diff.transpose();//Ref<概率机器人> page-53(9)
    }

    //------
    // calculated transformed covariance
    MatrixXt sigma = MatrixXt::Zero(N + K, K); //23*7
    for (int i = 0; i < ext_sigma_points.rows(); i++) {
      ...
      sigma += ext_weights[i] * (diffA * diffB.transpose());//Ref<概率机器人> page-53(10)
    }

    kalman_gain = sigma * expected_measurement_cov.inverse(); //卡尔曼增益//Ref<概率机器人> page-53(11)
...
public:
  //预测及测量更新mean   cov
  VectorXt mean;  ////p v q ab wb及其协方差阵,其中p q赋予初始值
  MatrixXt cov;
...
private:
   ...
   * @param sigma_points  calculated sigma points  predict 33,16   correct 47,23
   */
  void computeSigmaPoints(const VectorXt& mean, const MatrixXt& cov, MatrixXt& sigma_points) {
    const int n = mean.size();//16
    assert(cov.rows() == n && cov.cols() == n);

    Eigen::LLT<MatrixXt> llt;
    llt.compute((n + lambda) * cov);//Cholesky分解
    MatrixXt l = llt.matrixL();     //l为下三角矩阵

    sigma_points.row(0) = mean;
    for (int i = 0; i < n; i++) {//predict <16       correct <23
