...
class mapOptimization : public ParamServer {
 public:
  ...
  pcl::PointCloud<PointType>::Ptr
      laserCloudOri;  // lidar frame, not global frame
  ...
  void callback_laserCloudInfoHandler(
    ...
    if (timeLaserInfoCur - timeLastProcessing >=
        mappingProcessInterval)  // threshold 0.15
    {
      timeLastProcessing = timeLaserInfoCur;

      updateInitialGuess();  //根据imu或者initialGuess预测transformTobeMapped

      extractSurroundingKeyFrames();  // cloudKeyPoses3D为空时return  or only
                                      // invoke  extractNearby()
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void performLoopClosure() {
    ...
    *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
    ...
    Eigen::Affine3f
        correctionLidarFrame;  //当前位置相对回环使用的pre_cloud的坐标
    correctionLidarFrame = icp.getFinalTransformation();
    // transform from world origin to wrong pose
    Eigen::Affine3f tWrong = pclPointToAffine3f(
        copy_cloudKeyPoses6D->points[loopKeyCur]);  // current pose
    // transform from world origin to corrected pose
    // tCorrect 即corected_current
    // pose(类似点云坐标从lidar系左乘外参变化到imu系)
    ...
    gtsam::Pose3 poseFrom =
        Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(x, y, z));  // current pose
    gtsam::Pose3 poseTo = pclPointTogtsamPose3(
        copy_cloudKeyPoses6D->points[loopKeyPre]);  // pre pose
    ...
    loopPoseQueue.push_back(poseFrom.between(
        poseTo));  // poseFrom.between(poseTo) 相当于poseFrom.inverse * poseTo
                   // pre_history 相对 cur的坐标
  ...
  bool detectLoopClosureDistance(int* latestID, int* closestID) {
    ...
    for (int i = 0; i < (int)pointSearchIndLoop.size(); ++i) {
      ...
      if (abs(copy_cloudKeyPoses6D->points[id].time - timeLaserInfoCur) >
          historyKeyframeSearchTimeDiff)  // historyKeyframeSearchTimeDiff:30
                                          // default
      {
        loopKeyPre = id;
        break;
  ...
  void updateInitialGuess() {
    ...
    incrementalOdometryAffineFront =
        trans2Affine3f(transformTobeMapped);  //初始全为0
    ...
    if (cloudKeyPoses3D->points.empty()) {
      ...
      if (!useImuHeadingInitialization) transformTobeMapped[2] = 0;  // yaw
      //初始化为每帧点云start_frame对应的imu_msg的pose,即0，0,0 - imuRoll
      // imuPitch imuYaw
    ...
    if (cloudInfo.odomAvailable == true) {
      ...
      if (lastImuPreTransAvailable == false) {
        ...
        Eigen::Affine3f transIncre = lastImuPreTransformation.inverse() *
                                     transBack;  //后一帧点云相对前一帧的trans
        Eigen::Affine3f transTobe = trans2Affine3f(transformTobeMapped);
        Eigen::Affine3f transFinal =
            transTobe *
            transIncre;  //根据每帧点云的initialGuess预测的当前帧的pose
        pcl::getTranslationAndEulerAngles(
            transFinal, transformTobeMapped[3], transformTobeMapped[4],
            transformTobeMapped[5],  // x y z
            transformTobeMapped[0], transformTobeMapped[1],
            transformTobeMapped[2]);  // r p y
    ...
    if (cloudInfo.imuAvailable == true) {
      ...
      pcl::getTranslationAndEulerAngles(
          transFinal, transformTobeMapped[3], transformTobeMapped[4],
          transformTobeMapped[5],  // x y z r p y
          transformTobeMapped[0], transformTobeMapped[1],
  ...
  // void extractForLoopClosure()// useless currently,the invoker or the caller
  // is commented
  // {
  //     pcl::PointCloud<PointType>::Ptr cloudToExtract(new
  //     pcl::PointCloud<PointType>()); int numPoses = cloudKeyPoses3D->size();
  ...
  void extractNearby() {
    ...
    pcl::PointCloud<PointType>::Ptr surroundingKeyPosesDS(
        new pcl::PointCloud<
            PointType>());  //将当前位置邻近的pose降采样到1m范围内一个pose
  ...
  void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract) {
    ...
    for (int i = 0; i < (int)cloudToExtract->size(); ++i) {
      ...
      int thisKeyInd =
          (int)cloudToExtract->points[i]
              .intensity;  // size of cloudKeyPoses 3D and 6D,which is same
      if (laserCloudMapContainer.find(thisKeyInd) !=
        ...
        *laserCloudSurfFromMap += laserCloudMapContainer[thisKeyInd].second;
        ...
        // transformed cloud not available                         the size of
        // cornerCloudKeyFrames and cloudKeyPoses 3D-6D is same
    ...
    if (laserCloudMapContainer.size() > 1000)
      laserCloudMapContainer
          .clear();  // aLoopIsClosed == true时 correctPoses内clear()
  }
...
#pragma omp parallel for num_threads(numberOfCores)
    for (int i = 0; i < laserCloudCornerLastDSNum; i++) {
      ...
      pointAssociateToMap(&pointOri,
                          &pointSel);  // pointOri左乘transPointAssociateToMap
                                       // pointSel为global坐标系
      //在laserCloudCornerFromMapDS
      //即MapCornerCloud(global坐标系)中找pointSel的临近点
      ...
      if (pointSearchSqDis[4] < 1.0) {
        ...
        if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1)) {
          ...
          float ld2 =
              a012 / l12;  //计算点(x0,y0,z0)到直线(x1,y1,z1)、(x2,y2,z2)的距离
          ...
          // ld2越大,s越小    若s太小或者为负数,则本匹配点失效
          // 对之后优化不贡献约束 la lb lc
          // 为点到直线距离相对于点(x0,y0,z0)坐标的偏导数.
          if (s > 0.1) {
...
#pragma omp parallel for num_threads(numberOfCores)
    for (int i = 0; i < laserCloudSurfLastDSNum; i++) {
      ...
      if (pointSearchSqDis[4] < 1.0) {
        ...
        if (planeValid) {
          float pd2 = pa * pointSel.x + pb * pointSel.y + pc * pointSel.z +
                      pd;  //点pointSel到面的距离
  ...
  void scan2MapOptimization() {
    ...
    if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum &&
      ...
      kdtreeCornerFromMap->setInputCloud(
          laserCloudCornerFromMapDS);  // laserCloudCornerFromMapDS经过cloudKeyPoses6D的转换和拼接
      kdtreeSurfFromMap->setInputCloud(
          laserCloudSurfFromMapDS);  //同laserCloudCornerFromMapDS
  ...
  void transformUpdate() {
    ...
    transformTobeMapped[0] = constraintTransformation(
        ...
        rotation_tollerance);  // rotation_tollerance: 1000
    transformTobeMapped[1] =
        constraintTransformation(transformTobeMapped[1], rotation_tollerance);
    transformTobeMapped[5] = constraintTransformation(
        transformTobeMapped[5], z_tollerance);  // z_tollerance: 1000

    incrementalOdometryAffineBack =
        trans2Affine3f(transformTobeMapped);  //更新roll pitch z
  }
  ...
  void addOdomFactor() {
    if (cloudKeyPoses3D->points.empty()) {
      ...
      initialEstimate.insert(
          0, trans2gtsamPose(
                 transformTobeMapped));  // initialEstimate类型为gtsam::Values
    } else {
    ...
    LOG(INFO) << std::fixed << std::setprecision(6)
              << "add odom-factor transformTobeMapped:"
  ...
  void addLoopFactor() {
    ...
    for (int i = 0; i < (int)loopIndexQueue.size(); ++i) {
      ...
      gtsam::Pose3 poseBetween =
          loopPoseQueue[i];  // pre_history 相对 cur的坐标   cur->pre_history
  ...
  void saveKeyFramesAndFactor()  // cloudKeyPoses3D->push_back
  {
    ...
    cloudKeyPoses3D->push_back(
        thisPose3D);  //***********************************cloudKeyPoses3D->push_back
                      // only here
    ...
    cloudKeyPoses6D->push_back(
        thisPose6D);  //***********************************cloudKeyPoses6D->push_back
                      // only here
    ...
    // endl;
    //最新估计的pose的协方差矩阵
    poseCovariance = isam->marginalCovariance(isamCurrentEstimate.size() -
                                              1);  //类型: Eigen::MatrixXd
    ...
    LOG(INFO) << std::fixed << std::setprecision(6)
              << "optimized by gtsam transformTobeMapped:"
              << transformTobeMapped[3] << ", " << transformTobeMapped[4]
              << ", " << transformTobeMapped[5] << "--"
              << transformTobeMapped[0] << ", " << transformTobeMapped[1]
              << ", " << transformTobeMapped[2];
  ...
  void publishOdometry() {
    ...
    laserOdometryROS.header.frame_id = odometryFrame;  // odom
    ...
    laserOdometryROS.pose.pose.orientation =
        tf::createQuaternionMsgFromRollPitchYaw(transformTobeMapped[0],
                                                transformTobeMapped[1],
                                                transformTobeMapped[2]);
    // pub-topic "lio_sam/mapping/odometry"
    ...
    if (lastIncreOdomPubFlag == false) {
      ...
      if (cloudInfo.imuAvailable == true) {
        if (std::abs(cloudInfo.imuPitchInit) < 1.4) {
          ...
          imuQuaternion.setRPY(cloudInfo.imuRollInit, cloudInfo.imuPitchInit,
                               0);
          tf::Matrix3x3(transformQuaternion.slerp(imuQuaternion, imuWeight))
              .getRPY(rollMid, pitchMid, yawMid);
          roll = rollMid;
    ...
    // pub-topic:"lio_sam/mapping/odometry_incremental"
  ...
  void publishFrames() {
    ...
    publishCloud(&pubKeyPoses, cloudKeyPoses3D, timeLaserInfoStamp,
                 odometryFrame);  //即global path
    // Publish surrounding key frames
    publishCloud(&pubRecentKeyFrames, laserCloudSurfFromMapDS,
                 timeLaserInfoStamp,
                 odometryFrame);  // "lio_sam/mapping/map_local"
    ...
    if (pubRecentKeyFrame.getNumSubscribers() != 0) {
      ...
      *cloudOut += *transformPointCloud(laserCloudSurfLastDS, &thisPose6D);
      publishCloud(&pubRecentKeyFrame, cloudOut, timeLaserInfoStamp,
                   odometryFrame);  // "lio_sam/mapping/cloud_registered"
    }
    ...
    if (pubCloudRegisteredRaw.getNumSubscribers() != 0) {
      ...
      *cloudOut = *transformPointCloud(cloudOut, &thisPose6D);
      publishCloud(&pubCloudRegisteredRaw, cloudOut, timeLaserInfoStamp,
                   odometryFrame);  // "lio_sam/mapping/cloud_registered_raw"
    }
    // publish path
