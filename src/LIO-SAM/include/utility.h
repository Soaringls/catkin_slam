...
class ParamServer {
 public:
  ...
  ParamServer() {
    ...
    nh.param<std::string>("lio_sam/inspvaTopic", inspvaTopic,
    ...
    nh.param<bool>("lio_sam/use_demo", use_demo, true);
  ...
  sensor_msgs::Imu imuConverter(const sensor_msgs::Imu &imu_in) {
    ...
    Eigen::Quaterniond q_final = q_from * extQRPY;  // todo?? ls
  ...
  Eigen::Vector3d ToYPRAngles(const Eigen::Matrix3d &r) {
    Eigen::Vector3d ypr;
    if (r(2, 0) < 1) {
      if (r(2, 0) > -1) {
  ...
  void inspva2rosRPY(
    ...
    Eigen::Quaterniond q(insmsg.odom_pose.pose.orientation.w,
                         ...
                         insmsg.odom_pose.pose.orientation.y,
    ...
    // LOG(INFO) << "aaa0: w" << insmsg.odom_pose.pose.orientation.w << ", "
    //           << insmsg.odom_pose.pose.orientation.x;
...
void publishRawPoints(ros::Publisher *pub,
                      const sensor_msgs::PointCloud2ConstPtr &rawmsg,
                      std::string &frame) {
  sensor_msgs::PointCloud2 msg = *rawmsg;
