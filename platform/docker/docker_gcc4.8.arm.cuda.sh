...
function main(){
    ...
    ${DOCKER_CMD} run -it \
        ...
        --net bridge \
        -w /autobot \
        --add-host autobot_docker:127.0.0.3\
        --add-host autobot_test:127.0.0.2\
        --add-host ${LOCAL_HOST}:127.0.0.1 \
        --hostname autobot_test \
        --shm-size 2G \
        --pid=host \
    ...
    if [ "${USER}" != "root" ]; then
        docker exec $AUTOBOT_CYBER bash -c '/autobot/platform/docker/docker_adduser.sh'
